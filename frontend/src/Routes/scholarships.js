    ...
    <div className="scholarshipBody">
      ...
      <div className="scholarshipSearch">
        <div className="scholarshipSearchOutline">
          ...
          <input type="text" placeholder="Enter your search" value={search} onChange={update}/>
        </div>
      </div>
      ...
      <div className="scholarshipCardContainer">
        {!load ? apiData.filter(data => {return data.title.toLowerCase().includes(search)}).map((data) => (
          <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="scholarshipCardBody">
            ...
            <p><span className="scholarshipCardSubtitle">Benefits: </span>{data.benefits}</p>
            <p><span className="scholarshipCardSubtitle">Contact: </span>{data.contact}</p>
            <p><span className="scholarshipCardSubtitle">Website: </span><a href={data.website} target="_blank" rel="noreferrer " className="scholarshipRedirects">{data.website}</a></p>
          </motion.div>
        )
